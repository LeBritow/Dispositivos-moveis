...
story.append(bullets_styled([
    ...
    'Dashboard: soma automática de kW em uso, limite ajustável via Slider (padrão 22 kW, faixa 5–60), cartão de status com semáforo (verde/amarelo/vermelho); atualização automática a cada 2 segundos.',
...
tbl = Table([
    ...
    [Paragraph('2. Screen2 (Painel)', CELL), Paragraph('Tela central: cartão de status (verde/amarelo/vermelho), barra de carga, total em kW, limite, vagas e botões de ação.', CELL),
     Paragraph('CardStatus, CardUso, CardAcoes, Slider1 (ajuste do limite, 5–60 kW), ListPicker1 (recargas ativas), Buttons Nova Recarga/Encerrar/Histórico, Notifier1, TextToSpeech1, Clock1', CELL)],
...
story.append(code_block([
    ...
    'when Slider1.PositionChanged (thumbValue)',
    '    global limite = thumbValue',
    '    Label6.Text = "Limite: " + limite + " kW"',
    '    TinyDB1.StoreValue "limiteKw" limite   -> limite ajustado fica salvo',
    '    (o painel reavalia o semaforo na proxima execucao de AtualizarPainel)',
...
story.append(bullets_styled([
    ...
    '3. Na Screen2 (Painel): VerticalArrangements para os cartões CardStatus/CardUso/CardAcoes (BackgroundColor branco), Slider1 para ajustar o limite (Enabled marcado, MinValue 5, MaxValue 60), ListPicker1, botões, Notifier1, TextToSpeech1, Clock1 e TinyDB1. Implementar 4.2.',
